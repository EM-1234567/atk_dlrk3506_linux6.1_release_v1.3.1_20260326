...
/* tkl_wakeup.h (deep-sleep wakeup source) is only used under ENABLE_LOW_POWER
 * and is not provided by all vendor adapters (e.g. RK3506B Linux). Guard it so
 * the build doesn't pull a header that doesn't exist when low-power is off. */
 ...
 * Macros
 * --------------------------------------------------------------------------- */
#define AI_DEVICE_VOLUME      "ai_dev_volume"   /* 1 byte: volume 0~100 */
#define AI_DEVICE_MODE        "ai_dev_mode"     /* 1 byte: [7:4]=AI_DEVICE_MODE_E, [3:0]=AI_CHAT_SUB_MODE_E */

#define AI_DEVICE_MODE_PACK(dev, sub) \
    ((UINT8_T)((((UINT8_T)(dev) & 0x0F) << 4) | ((UINT8_T)(sub) & 0x0F)))
...
/**
 * @brief Save volume to KV (AI_DEVICE_VOLUME), 1 raw byte.
 * @return OPRT_OK on success, error code on failure.
 */
STATIC OPERATE_RET __ai_toy_volume_save(VOID)
{
    UINT8_T v = s_ai_toy->volume;
    TAL_PR_DEBUG("save ai_toy volume: %u", v);
    return wd_common_write(AI_DEVICE_VOLUME, (CONST BYTE_T *)&v, sizeof(v));
}

/**
 * @brief Save device mode + chat sub-mode to KV (AI_DEVICE_MODE), packed in 1 byte.
 * @param[in] dev Target device mode.
 * @param[in] sub Target chat sub-mode.
 * @return OPRT_OK on success or when skipped (P2P/RECORD), error code on failure.
 * @note When dev is P2P or RECORD, persistence is skipped intentionally so that
 *       these transient modes do not survive a reboot.
 */
STATIC OPERATE_RET __ai_toy_mode_save(AI_DEVICE_MODE_E dev, AI_CHAT_SUB_MODE_E sub)
{
    if (dev < AI_DEVICE_MODE_CHAT || dev >= AI_DEVICE_MODE_MAX ||
        sub < AI_CHAT_SUB_HOLD   || sub >= AI_CHAT_SUB_MAX) {
        TAL_PR_ERR("ai toy -> mode save invalid param, dev:%d, sub:%d", dev, sub);
        return OPRT_INVALID_PARM;
    }

    if (dev == AI_DEVICE_MODE_P2P || dev == AI_DEVICE_MODE_RECORD) {
        TAL_PR_DEBUG("ai toy -> skip mode save for transient mode %d", dev);
        return OPRT_OK;
    }

    UINT8_T packed = AI_DEVICE_MODE_PACK(dev, sub);
    TAL_PR_DEBUG("save ai_toy mode: dev=%d sub=%d packed=0x%02X", dev, sub, packed);
    return wd_common_write(AI_DEVICE_MODE, (CONST BYTE_T *)&packed, sizeof(packed));
}

/**
 * @brief Load volume from KV; fall back to TY_SPK_DEFAULT_VOL on miss / invalid data.
 * @return OPRT_OK on success.
 */
STATIC OPERATE_RET __ai_toy_volume_load(VOID)
{
    BYTE_T *value = NULL;
    UINT_T  len   = 0;
    ...
    if (OPRT_OK != wd_common_read(AI_DEVICE_VOLUME, &value, &len) || value == NULL) {
        return OPRT_OK;
    }

    if (len >= 1 && value[0] <= 100) {
        s_ai_toy->volume = value[0];
    } else {
        TAL_PR_WARN("ai toy -> invalid volume in kv, len=%u, val=%u", len, (len >= 1) ? value[0] : 0);
    }
...
/**
 * @brief Load device_mode + trigger_mode from KV; keep init defaults on miss / invalid data.
 * @return OPRT_OK on success.
 * @note P2P/RECORD must never be persisted; if seen, fall back to AI_DEVICE_MODE_CHAT.
 */
STATIC OPERATE_RET __ai_toy_mode_load(VOID)
{
    BYTE_T *value = NULL;
    UINT_T  len   = 0;

    if (OPRT_OK != wd_common_read(AI_DEVICE_MODE, &value, &len) || value == NULL) {
        return OPRT_OK;
    }

    if (len < 1) {
        TAL_PR_WARN("ai toy -> empty mode kv, len=%u", len);
        wd_common_free_data(value);
        return OPRT_OK;
    }

    UINT8_T            packed = value[0];
    AI_DEVICE_MODE_E   dev    = AI_DEVICE_MODE_DEV(packed);
    AI_CHAT_SUB_MODE_E sub    = AI_DEVICE_MODE_SUB(packed);
    wd_common_free_data(value);

    if (dev == AI_DEVICE_MODE_P2P || dev == AI_DEVICE_MODE_RECORD ||
        dev <  AI_DEVICE_MODE_CHAT || dev >= AI_DEVICE_MODE_MAX) {
        TAL_PR_WARN("ai toy -> invalid stored device_mode %d, fallback to CHAT", dev);
        dev = AI_DEVICE_MODE_CHAT;
    }
    s_ai_toy->cfg.device_mode = dev;

    if (sub < AI_CHAT_SUB_HOLD || sub >= AI_CHAT_SUB_MAX) {
        TAL_PR_WARN("ai toy -> invalid stored chat_sub_mode %d, keep init default", sub);
    } else {
        s_ai_toy->cfg.trigger_mode = sub;
    }
...
/**
 * @brief Load volume and mode from KV; use defaults if read fails.
 * @return OPRT_OK on success.
 * @note Only called once during tuya_ai_toy_init().
 */
STATIC OPERATE_RET __ai_toy_config_load(VOID)
{
    TAL_PR_NOTICE("ai toy -> load config");

    __ai_toy_volume_load();
    __ai_toy_mode_load();

    TAL_PR_DEBUG("load ai_toy config: volume=%u, device_mode=%d, trigger_mode=%d",
                 s_ai_toy->volume, s_ai_toy->cfg.device_mode, s_ai_toy->cfg.trigger_mode);
    return OPRT_OK;
}
...
STATIC VOID __on_ai_toy_wf_nw_stat_cb(GW_WIFI_NW_STAT_E nw_stat)
{
    ...
    case STAT_UNPROVISION_AP_STA_UNCFG:
        /* Unprovisioned SoftAP: show netconfig UI, LED flash 200 ms. */
#if defined(ENABLE_TUYA_UI)
...
{
    wd_common_delete(AI_DEVICE_VOLUME);
    wd_common_delete(AI_DEVICE_MODE);
    return OPRT_OK;
}
...
// {
//     /* Session creation moved to Tuya Provider (__tuya_on_client_run). */
//     (VOID_T)data;
// }

/** AI client connected: forward to wukong mode dispatch. */
...
STATIC OPERATE_RET __ai_toy_wukong_ai_agent_init(VOID)
{
    ...
    audio_cfg.board.vad_active_ms  = 500;
...
VOID tuya_ai_toy_trigger_mode_set(AI_CHAT_SUB_MODE_E mode)
{
    if (mode >= AI_CHAT_SUB_HOLD && mode < AI_CHAT_SUB_MAX) {
        if (s_ai_toy->cfg.trigger_mode != mode) {
            s_ai_toy->cfg.trigger_mode = mode;
            __ai_toy_mode_save(s_ai_toy->cfg.device_mode, s_ai_toy->cfg.trigger_mode);
        }
        return;
    }
...
VOID tuya_ai_toy_device_mode_set(AI_DEVICE_MODE_E mode)
{
    if (mode >= AI_DEVICE_MODE_CHAT && mode < AI_DEVICE_MODE_MAX) {
        if (s_ai_toy->cfg.device_mode != mode) {
            s_ai_toy->cfg.device_mode = mode;
            /* P2P/RECORD will be filtered out inside the save helper. */
            __ai_toy_mode_save(s_ai_toy->cfg.device_mode, s_ai_toy->cfg.trigger_mode);
        }
        return;
    }
...
{
    if (value > 100) {
        TAL_PR_ERR("ai toy -> volume set error, value:%u", value);
        return OPRT_INVALID_PARM;
    }

    s_ai_toy->volume = value;

    OPERATE_RET rt = __ai_toy_volume_save();
    if (rt != OPRT_OK) {
        TAL_PR_ERR("ai toy -> save volume failed, rt:%d", rt);
    }
