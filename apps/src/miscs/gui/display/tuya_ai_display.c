...
/* UI action queue payload allocator: payload is heap-copied on post and
 * freed by the consumer thread, so it must outlive the calling stack
 * frame. Aligns with ENABLE_EXT_RAM convention used elsewhere in this
 * file (e.g. disp_msg_queue path also stores PSRAM-backed data). */
#if defined(ENABLE_EXT_RAM) && (ENABLE_EXT_RAM == 1)
#define UI_ACT_MALLOC(s) tkl_system_psram_malloc(s)
#define UI_ACT_FREE(p)   tkl_system_psram_free(p)
#else
#define UI_ACT_MALLOC(s) tal_malloc(s)
#define UI_ACT_FREE(p)   tal_free(p)
#endif
...
STATIC VOID_T __tuya_ai_action_thread(VOID_T *arg)
{
    ...
    for (;;) {
        ...
        /* Payload was deep-copied in tuya_ai_display_action_post and is
         * owned by this consumer; callbacks must not retain msg pointer. */
        if (msg.msg != NULL) {
            UI_ACT_FREE(msg.msg);
            msg.msg = NULL;
        }
...
OPERATE_RET tuya_ai_display_action_post(UINT8_T *msg, INT_T len, TY_DISPLAY_ACTION_E disp_action)
{
    ...
    /* Deep-copy payload so it survives the queue hand-off across
     * threads. Caller-owned stack buffers would otherwise be reclaimed
     * before the consumer thread reads them. */
    if (msg != NULL && len > 0) {
        copy = (UINT8_T *)UI_ACT_MALLOC((SIZE_T)len);
        if (copy == NULL) {
    ...
    act_msg.action = disp_action;
    act_msg.msg    = copy;
    act_msg.len    = len;

    rt = tkl_queue_post(s_ui_action_queue, &act_msg, 0);
    if (rt != OPRT_OK && copy != NULL) {
        UI_ACT_FREE(copy);
    }
    return rt;
}
