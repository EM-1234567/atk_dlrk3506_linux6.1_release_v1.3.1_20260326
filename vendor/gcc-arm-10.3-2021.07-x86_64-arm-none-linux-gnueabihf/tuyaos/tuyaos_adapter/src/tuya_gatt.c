...
static void chr_set_value(const GDBusPropertyTable *property, DBusMessageIter *iter, GDBusPendingPropertySet id, void *user_data)
{
    ...
    printf("Characteristic(%s): Set('Value', ...)", chr->uuid);
...
static const GDBusPropertyTable chr_properties[] = {{"UUID", "s", chr_get_uuid}, {"Service", "o", chr_get_service}, {"Value", "ay", chr_get_value, chr_set_value, NULL}, {"Flags", "as", chr_get_props, NULL, NULL}, {}};
...
static DBusMessage *chr_write_value(DBusConnection *conn, DBusMessage *msg, void *user_data)
{
    ...
    if (__gatt_write_request_event) {
        printf("__gatt_write_request_event-----------------------------\n");
...
static void register_app_reply(DBusMessage *reply, void *user_data)
{
    ...
    if (dbus_error_is_set(&derr))
        printf("RegisterApplication: %s", derr.message);
    else
...
{
    if (!g_dbus_proxy_method_call(proxy, "RegisterApplication", register_app_setup, register_app_reply, NULL, NULL)) {
...
static void property_changed_cb(GDBusProxy *proxy, const char *name, DBusMessageIter *iter, void *user_data)
{
    ...
    printf("property_changed, path: %s, iface: %s, name: %s", path, interface, name);
...
int tuya_gatt_init(void)
{
    ...
    g_dbus_client_set_proxy_handlers(client, proxy_added_cb, NULL, property_changed_cb, NULL);
