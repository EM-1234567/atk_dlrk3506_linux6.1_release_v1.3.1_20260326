...
/**
 * @brief heart range proc
 *
 * @param[in] result cloud heart range info
 *
 * @return ackId
 */
CHAR_T* mqc_heart_range_proc(ty_cJSON* result);

/**
 * @brief get mqtt heart interval
 *
 * @param[in] le_connect Less than or Equal mqtt connect
 * @param[in] hb_lost_cnt heart beat lost count
 *
 * @return heart interval
 */
UINT_T mqc_get_heart_interval(BOOL_T le_connect, BYTE_T hb_lost_cnt);

/**
 * @brief sync mqtt heart range
 *
 * @return OPRT_OK on success. Others on error, please refer to tuya_error_code.h
 */
OPERATE_RET mqc_sync_heart_range();
#ifdef __cplusplus
}
#endif
