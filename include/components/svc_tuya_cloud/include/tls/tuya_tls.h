...
/**
 * @brief number of already-decrypted application bytes buffered inside the TLS
 *        context, readable without touching the socket (mbedtls_ssl_get_bytes_avail).
 *
 * @param[in] tls_handler refer to tuya_tls_hander
 *
 * @return buffered byte count (0 if none or handler is NULL)
 */
INT_T tuya_tls_get_bytes_avail(IN tuya_tls_hander tls_handler);
...
/**
 * @brief set tls calloc and free function for mbedtls
 * @param[in] calloc_fn custom calloc (NULL to use default Tuya alloc)
 * @param[in] free_fn custom free (NULL to use default Tuya free)
 */
void tuya_tls_set_calloc_and_free(void *(*calloc_fn)(SIZE_T nmemb, SIZE_T size), void (*free_fn)(void *p));
