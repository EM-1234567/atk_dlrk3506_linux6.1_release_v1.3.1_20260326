...
/**
 * @brief Check product secret
 *
 * @return TRUE on product secret
 */
BOOL_T tuya_svc_devos_activate_is_product_secret(VOID);

/**
 * @brief Call activate preaction
 *
 * @return OPRT_OK on success. Others on error, please refer to tuya_error_code.h
 */
...
/**
 * @brief set tuyaos auto recover
 *
 * @param[in] short_time short time out value of netcfg(min 10s).
 * @param[in] long_time long time out value of netcfg(min 60s).
 *
 * @return OPRT_OK on success. Others on error, please refer to tuya_error_code.h
 */
OPERATE_RET tuya_svc_devos_set_auto_recover(INT_T short_time, INT_T long_time);
/**
