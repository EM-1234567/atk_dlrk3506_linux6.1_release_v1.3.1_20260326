...
* @param[in] mode: dns mode, DNS_IPV4 or DNS_IPV6
* @param[out] addr: address information
*
* @note This API is used for getting address information by domain.
*
* @return OPRT_OK on success. Others on error, please refer to tuya_error_code.h
*/
OPERATE_RET tal_net_gethostbyname_p(IN CONST CHAR_T *domain, IN CONST DNS_MODE_E mode, OUT TUYA_IP_ADDR_T *addr);

#define tal_net_gethostbyname(domain, addr) tal_net_gethostbyname_p(domain, DNS_IPV4, addr)
...
/**
* @brief Get local socket address and port by fd
*
* @param[in] fd: file descriptor
* @param[out] addr: ip address
* @param[out] port: port in host byte order
*
* @note This API is used to get the current name for the specified socket.
*
* @return 0 on success, negative value on error
*/
INT_T tal_net_getsockname(INT_T fd, TUYA_IP_ADDR_T *addr, INT_T *port);
...
* @return OPRT_OK on success. Others on error, please refer to tuya_error_code.h
*/
OPERATE_RET tal_net_set_default_netif_by_ip(const TUYA_IP_ADDR_T addr);
