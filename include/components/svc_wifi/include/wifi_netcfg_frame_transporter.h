...
uint8_t wifiNetcfgGetChannelOnce(char *ssid);

/*
    获取当前缓存的扫描到的AP列表（不转移所有权，调用方不要释放）；
    若尚未扫描，则触发一次 wifiNetcfgScanAll。
    返回的指针在 wifiNetcfgGetChannelOnce 调用后会失效。
*/
int wifiNetcfgGetApList(AP_IF_S **ppApArray, uint32_t *pApNum);
