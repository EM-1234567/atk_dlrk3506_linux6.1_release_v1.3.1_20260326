...
/**
 * @brief Check if the ble stack is initialized
 *
 * @return TRUE if the ble stack is initialized, FALSE otherwise
 */
BOOL_T tuya_is_ble_stack_init(VOID_T);

/**
 * @brief Set the extended advertisement parameters
 *
 * @param[in] adv_interval_min: the minimum advertisement interval
 * @param[in] adv_interval_max: the maximum advertisement interval
 */
VOID_T tuya_sdk_set_ext_adv_params(uint16_t adv_interval_min, uint16_t adv_interval_max);
#if defined(ENABLE_MATTER) && (ENABLE_MATTER == 1)
void tuya_ble_parse_scan_info(TAL_BLE_ADV_REPORT_T *scan_info);
#endif

/**
 * @brief Set the mask for BLE LP status decision
 *
 * @param[in] mask  bitmask: bit0-adv, bit1-conn, bit2-scan.
 *                  Only masked bits participate in LP enter/exit decision.
 *                  e.g. 0x02 means only BLE connection affects LP.
 *
 * @return VOID
 */
VOID_T tuya_sdk_bt_set_lp_stat_mask(UINT8_T mask);
