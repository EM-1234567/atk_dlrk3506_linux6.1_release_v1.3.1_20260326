...
#define WEBSOCKET_MAX_HEADERS           8

typedef struct {
    char *key;
    char *value;
} websocket_client_header_t;
...
typedef struct {
    ...
    char                            *token;
    websocket_client_header_t       headers[WEBSOCKET_MAX_HEADERS];
...
int websocket_client_add_header(websocket_client_handle_t handle, const char *key, const char *value);
